...
private_lane :rsb_start_release_branch do |options|
  ...
  sh("git checkout -b release/#{name}") { |ok, res|
    break if !!ok
    UI.error("Could not create release branch with name #{name}. Please choose another one.")
...
private_lane :rsb_end_release_branch do |options|
  ...
  begin
    ...
    UI.error("An error occured while trying to merge release branch: \"#{e}\"")
  end
...
private_lane :rsb_remove_release_branch do |options|
  ...
  checkout_needed_branch_after_completion
  sh("git branch -D release/#{name}") {
    UI.error("Could not remove release branch")
  }
end

def checkout_needed_branch_after_completion
  sh("git checkout #{$checkout_branch}") {
      UI.error("Could not checkout to branch #{$checkout_branch}")
  }
end
...
private_lane :rsb_finish_hotfix_branch do |options|
  ...
  begin
    ...
    UI.error("An error occured while trying to merge hotfix branch: \"#{e}\"")
  end
end
...
private_lane :rsb_delete_hotfix_branch do |options|
  ...
  begin
    ...
    UI.error("An error occured while trying to delete hotfix branch: \"#{e}\"")
  end
end
